...
import com.chamsion.filemanager.bean.entity.ApkInfo;
...
public class FileUtil {
    ...
    public static ArrayList<String> listFiles(String path, Context c) {
        ...
        if (file.exists()) {
            ...
            //判断该文件是否为空
            if (files == null) {
                mDirContent.add(file.getAbsolutePath());
...
//                ToastUtil.showToast(c, "无法读取此目录");
            } else if (files.length == 0) {
                ToastUtil.showToast(c, "没有读取到内容");
            } else {
                for (File tFile : files) {
                    if (!showhidden) {
                        if (!tFile.isHidden())
                            mDirContent.add(tFile.getAbsolutePath());
                    } else {
                        mDirContent.add(tFile.getAbsolutePath());
                    }
                }
...
//        }
    ...
    public static void openFile(final Context context, final File target) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            Uri contentUri = FileProvider.getUriForFile(context, "com.chamsion.filemanager" + ".fileprovider", target);
